...
while(1):
    ...
    try: 
        ...
        if k==1:
            ...
            path = "D:\\NUS\\ProjectSS\\data\\rgbd_bedroom\\bedroom\\image"
            i = 0
            for filename in os.listdir(path):
                if filename.endswith(".jpg") and i < 200: # increase value to view more frames 
                    if cv2.waitKey(1000000) & 0xFF == ord('k'): #press and hold k to view all the frames
                        path2=os.path.join(path,filename)
                        i = i + 1
                        print(path2)
                        abc = cv2.imread(path2)
